class ActivityLog < ApplicationRecord
  validates :stop_time_before_type_cast, format: { with: /\d{4}-[01]\d-[0-3]\dT[0-2]\d:[0-5]\d:[0-5]\d(?:\.\d+)?Z?/, message: "Formato de fecha no valido" }, allow_nil: true
  validates :start_time_before_type_cast, format: { with: /\d{4}-[01]\d-[0-3]\dT[0-2]\d:[0-5]\d:[0-5]\d(?:\.\d+)?Z?/, message: "Formato de fecha no valido" }, allow_nil: true
  ...
  #scope :filter_status, -> (status) { where.not stop_time: status }
  #scope :filter_status, lambda{ |name| self.where("name ILIKE ?", "%#{name}%") if name.present? }
